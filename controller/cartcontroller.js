...
const cartcontroller = {
  async createCart(req, res) {
    try {
      ...
      if (!User) {
        return res.status(400).json({ success: false, error: 'userId is missing in the request parameters' });
      }
  ...
  async getCart(req, res) {
    try {
      ...
      // Check if userId is undefined or empty
      if (!User) {
        return res.status(400).json({ success: false, error: 'userId is missing in the body' });
      }
  ...
  async updateCartTourDetail(req, res) {
    try {
      ...
      const updatedCartTourDetail = await Cartmodel.updateCartTourDetail(updatedData);
